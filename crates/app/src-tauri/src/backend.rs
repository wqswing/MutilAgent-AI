...
use multi_agent_skills::{DefaultToolRegistry, EchoTool, CalculatorTool, McpRegistry, CompositeToolRegistry, load_mcp_config};
...
pub async fn start_server() -> Result<()> {
    ...
    let make_writer = move || ChannelWriter { tx: tx_for_logs.clone() };
    
    let env_filter = tracing_subscriber::EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| "info,multi_agent=debug".into());

    if let Err(e) = tracing_subscriber::registry()
        .with(env_filter)
        .with(fmt::layer().with_writer(make_writer).json().flatten_event(true))
        .try_init() 
    {
    ...
    // Initialize L3: Artifact Store
    // =========================================================================
    let (store, artifacts_erasables, artifacts_prunables): (Arc<dyn ArtifactStore>, Vec<Arc<dyn Erasable>>, Vec<Arc<dyn Prunable>>) = if let Ok(bucket) = std::env::var("AWS_S3_BUCKET") {
    ...
    // Initialize Session Store
    let (session_store, session_erasable, session_prunable): (Arc<dyn SessionStore>, Arc<dyn Erasable>, Arc<dyn Prunable>) = if let Ok(redis_url) = std::env::var("REDIS_URL") {
        tracing::info!(url = %redis_url, "Initializing Redis Session Store");
        let s = Arc::new(RedisSessionStore::new(&redis_url, "multiagent:session", 3600 * 24)?);
        (s.clone(), s.clone(), s)
    } else {
    ...
    let sandbox_manager = match multi_agent_sandbox::DockerSandbox::new() {
        Ok(engine) => {
            ...
            local_registry.register(Box::new(multi_agent_sandbox::SandboxShellTool::new(manager.clone()))).await?;
            local_registry.register(Box::new(multi_agent_sandbox::SandboxWriteFileTool::new(manager.clone()))).await?;
            local_registry.register(Box::new(multi_agent_sandbox::SandboxReadFileTool::new(manager.clone()))).await?;
            local_registry.register(Box::new(multi_agent_sandbox::SandboxListFilesTool::new(manager.clone()))).await?;
    ...
    let network_policy = if std::path::Path::new(policy_path).exists() {
        match std::fs::read_to_string(policy_path) {
            Ok(content) => match serde_yaml::from_str::<multi_agent_governance::network::NetworkPolicy>(&content) {
                Ok(policy) => {
                    tracing::info!("Loaded network policy from {}", policy_path);
                    Arc::new(policy)
                }
                Err(e) => {
                     tracing::error!("Failed to parse network policy: {}. Using Deny-All default.", e);
                     Arc::new(multi_agent_governance::network::NetworkPolicy::default())
                }
            },
            Err(e) => {
                tracing::error!("Failed to read network policy: {}. Using Deny-All default.", e);
                Arc::new(multi_agent_governance::network::NetworkPolicy::default())
            }
        }
    } else {
        tracing::info!("No network policy found at {}. Using Deny-All default.", policy_path);
    ...
    local_registry.register(Box::new(multi_agent_skills::network::FetchTool::new(network_policy.clone()))).await?;
    
    if let Some(manager) = sandbox_manager {
        local_registry.register(Box::new(multi_agent_skills::network::DownloadTool::new(network_policy.clone(), manager))).await?;
    } else {
    ...
    let audit_log_path = std::env::var("AUDIT_LOG_PATH").unwrap_or_else(|_| ".sovereign_claw/audit.log".to_string());
    if let Some(parent) = std::path::Path::new(&audit_log_path).parent() {
        let _ = std::fs::create_dir_all(parent);
    }
    let audit_store = Arc::new(multi_agent_governance::FileAuditStore::new(&audit_log_path));
    ...
    let rbac = Arc::new(multi_agent_governance::StaticTokenRbacConnector::new(admin_token));
    let secrets = Arc::new(multi_agent_governance::AesGcmSecretsManager::new(None));

    let provider_store = Arc::new(multi_agent_store::FileProviderStore::new(".sovereign_claw/providers.json"));
...
"#;
    ...
    let policy_engine = match multi_agent_governance::PolicyEngine::load(&default_policy_path) {
        ...
        Err(e) => {
            ...
            Arc::new(tokio::sync::RwLock::new(multi_agent_governance::PolicyEngine::from_file(multi_agent_governance::PolicyFile {
                version: "0.0.0".into(),
    ...
    let audit_log_path_secure = std::env::var("AUDIT_LOG_STORAGE_PATH")
        .unwrap_or_else(|_| ".sovereign_claw/audit.log.jsonl".to_string());
    if let Some(parent) = std::path::Path::new(&audit_log_path_secure).parent() {
        let _ = std::fs::create_dir_all(parent);
    }
    let audit_subscriber = Arc::new(crate::audit_log::AuditSubscriber::new(&audit_log_path_secure)?);
    ...
    // =========================================================================
    // Already initialized above for PrivacyController
    ...
    // Onboarding Check
    let onboarding_completed = std::env::var("OPENAI_API_KEY").is_ok() 
        || std::env::var("ANTHROPIC_API_KEY").is_ok();
    
    if !onboarding_completed {
        tracing::warn!("⚠️  ONBOARDING REQUIRED: No LLM API keys found (OPENAI_API_KEY or ANTHROPIC_API_KEY)");
    }
    ...
    let llm_client: Arc<dyn LlmClient> = match multi_agent_model_gateway::create_default_client() {
        ...
        Err(e) => {
            tracing::warn!("Failed to create default LLM client: {}. Cache fallback.", e);
    ...
    let allowed_origins = match std::env::var("ALLOWED_ORIGINS") {
        Ok(val) => {
            ...
            if !cfg!(debug_assertions) && origins.contains(&"*".to_string()) {
                tracing::error!("FATAL: Wildcard CORS ('*') is forbidden in production builds.");
                return Err(anyhow::anyhow!("Secure Default Violation: Wildcard CORS in production"));
            }
            origins
        }
        Err(_) => {
            if cfg!(debug_assertions) {
                tracing::warn!("ALLOWED_ORIGINS not set. Defaulting to '*' for development.");
                vec!["*".to_string()]
            } else {
                tracing::error!("FATAL: ALLOWED_ORIGINS must be set in production builds.");
                return Err(anyhow::anyhow!("Secure Default Violation: ALLOWED_ORIGINS missing"));
            }
        }
    };
    ...
    tracing::info!(
        host = %config.host,
        port = config.port,
        "L0 Gateway initialized (Embedded)"
    );
