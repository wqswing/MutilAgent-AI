...
pub fn validate_sandbox_path(root: &str, input_path: &str) -> Result<PathBuf> {
            ...
            Component::ParentDir => {
                if !normalized.pop() {
                    return Err(crate::Error::SecurityViolation(format!(
                        "Path traversal detected in path: {}", input_path
                    )));
                }
            }
            Component::RootDir | Component::Prefix(_) => {
                return Err(crate::Error::SecurityViolation(format!(
                    "Absolute paths are not allowed in sandbox: {}", input_path
                )));
            }
            Component::CurDir => {}
        }
    ...
    if !full_path.starts_with(root_path) {
        return Err(crate::Error::SecurityViolation(format!(
            "Access denied: path {} is outside of root {}", input_path, root
        )));
    }
...
mod tests {
    ...
    #[test]
    fn test_valid_paths() {
        assert_eq!(validate_sandbox_path("/workspace", "main.py").unwrap(), PathBuf::from("main.py"));
        assert_eq!(validate_sandbox_path("/workspace", "src/app.js").unwrap(), PathBuf::from("src/app.js"));
        assert_eq!(validate_sandbox_path("/workspace", "./local.txt").unwrap(), PathBuf::from("local.txt"));
    }
